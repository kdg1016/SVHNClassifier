...
#   def imshow(img, desc='imshow', zoom=0.0, skip=False, pause_sec=0, loc=(10, 10)):
...
#   def imread_safe(img_file, color_fmt='RGB'):
...
#   def get_file_list(dir_name, ext='.pdf'):
#   def template_matching(ref_img, tar_img, method=cv2.TM_CCOEFF, show_=False):
#
...
def imshow(img, desc='imshow', zoom=0.0, skip=False, pause_sec=0, loc=(10,10)):
    ...
    zoom = get_full_zoom_factor(img, zoom=zoom, factor=4/5.)
...
def find_common_area(p, q):

    area1 = (p[2] - p[0]) * (p[3] - p[1])
    area2 = (q[2] - q[1]) * (q[3] - q[1])

    if p[2] < q[0] or q[2] < p[0] or p[3] < q[1] or q[3] < p[1]:
        vertex_pts = (0,0,0,0)
    else:
        vertex_pts = (max(p[0], q[0]), max(p[1], q[1]), min(p[2], q[2]), min(p[3], q[3]))
    '''
...
def to_str(string_dat):
    try:
        if isinstance(string_dat, unicode):
...
def to_unicode(unicode_dat):
    if isinstance(unicode_dat, str):
        try:
            ...
            unicode_dat = bytes(unicode_dat, 'utf-8')
    return unicode_dat
...
def imread_safe(img_file, color_fmt='RGB'):
    """
    Read image with many exception handlings.
...
'''
    if False:   # Binary image test
        ...
        for method in methods:
            ...
            me.imshow(ref_img, desc=method, zoom=1.0)
'''
...
def read_pdf(pdf_filename, resolution=300):
    ...
    img = imread_safe(img_filename, color_fmt='RGB')
...
def get_file_list(dir_name, ext='.pdf', sorting_=True):
    ...
    if os.path.isdir(dir_name):
        file_list = list(paths.list_files(dir_name, validExts=tuple(ext), contains=None))
        if sorting_:
            file_list.sort()
...
def get_color_histogram(img_file, type='RGB'):

    img = imread_safe(img_file)

    if type.lower() == 'gray':
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        hist = cv2.calcHist([gray], [0], None, [256], [0,256])
        plt_show(hist, "Grayscale histogram", "Bins", "# of pixels", [0, 256], None)
    elif type.lower() == 'rgb':
...
def derotate_img(img, compare_imgs=0, line_img_file=None, rot_img_file=None, check_lines_in_img_=False, check_time_=False):
    ...
    if compare_imgs is not 0:
        ...
        imshow(np.concatenate((line_img, rot_img), axis=1), pause_sec=compare_imgs)
...
def compare_strings(string1, string2, no_match_c='*', match_c='|'):
    ...
    result = ''
    n_diff = 0
    # for c1, c2 in itertools.izip(string1, string2):   # Python 2.
    for c1, c2 in zip(string1, string2):                # Python 3.
        if c1 == c2:
            result += match_c
        else:
            result += no_match_c
...
def template_matching(tar_full_img, tmp_full_img, tar_area=None, tmp_area=None, method=cv2.TM_CCOEFF, pause=0):
    """
    Template matching
    ...
    if pause > 0:
        ...
        merge_img[:dim_tmp[0]:,:dim_tmp[1],:] = tmp_img[:,:,:]
        merge_img[dim_tmp[0]+bar_num:,:,:] = tar_box_img[:,:,:]
        imshow(merge_img, desc='imshow', pause_sec=pause, loc=(10,10))
        # plt_imshow2(tmp_img, tar_box_img, title=(str(min_val), str(max_loc)), block=2)
...
def raw_input_safe(in_str):
    ...
    if sys.version_info > (3,0):
        ...
        return raw_input(in_str)


def check_directory_existence(in_dir, exit_=False, create_=False):
    """
...
def read_all_images(img_dir, prefix='', exts=IMG_EXTS, color_fmt='RGB', apply_derot_=False, pause_derot_sec=0):
    """Read all images with specific filename prefix in an image directory.

    :param img_dir:
    :param prefix:
    :param apply_derot_:
    :param pause_derot_sec:
    :return:
    """
    imgs = []
    ...
    for filename in filenames:
        if filename.startswith(prefix) and os.path.splitext(filename)[-1][1:] in exts:
            img = imread_safe(os.path.join(img_dir, filename), color_fmt=color_fmt)
            if apply_derot_:
                imgs.append(derotate_img(img, compare_imgs=pause_derot_sec))
            else:
                imgs.append(img)
...
def draw_box_overlay(img, box, color, alpha=0.5):
    """
    ...
    :return:
    """
    over = cv2.rectangle(deepcopy(img), tuple(box[0:2]), tuple(box[2:]), color, -1)
    over = cv2.addWeighted(img, alpha, over, 1 - alpha, 0)
    return over
